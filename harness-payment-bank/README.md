...
| Delegate logs `401 ACCOUNT_DOES_NOT_EXIST` on every call, delegate never registers | `harness_platform_delegatetoken` had `org_id` set, making the token **org-scoped**, while the Helm chart registers at **account** level with `accountId` only. The delegate installed by hand from account-level settings works because its token is account-scoped | Drop `org_id` from the token resource. `manager_endpoint` is *not* the cause — `app.harness.io` is correct for this account. `decode_delegate_token = true` is also correct: the provider documents `value` as base64-encoded |
